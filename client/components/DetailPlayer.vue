      ...
      <div class="detail_player-lrc">
        <div class="lrc-content" :style="{marginTop: lrcOffset + 'px' }">
          <p v-for="(item,index) in songLrc"
             :class="{showable: item.seconds <= audio.currentLength}">
            {{item.lrcContent}}</p>
        </div>
      </div>
...
<script type="es6">
  ...
  export default {
    ...
    computed: {
      ...
      lrcOffset() {
        if (this.songLrc) {
          const showableEles = document.querySelectorAll('.showable')
          const offset = (showableEles.length - 2) * (-20)
          for(let ele of showableEles) {
            ele.className = 'showable'
          }
          if(showableEles[showableEles.length - 1]) {
            showableEles[showableEles.length - 1].className = 'showable current'
          }
